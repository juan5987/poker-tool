...
//style
import './chipsChipElement.scss';

const ChipsChipElement = ({
    color, 
    value, 
    quantity, 
    handleChipsFormInput,
    chipColor,
    handleRemoveChip,
}) => (
    <li className="chipElement">
        ...
        <div className="chipElement__field">
            ...
            <input onChange={handleChipsFormInput} type="color" name="color" className="chipElement__field__inputColor" value={color} required/>
        </div>

        <div className="chipElement__field">
            <label className="chipElement__field__label">Valeur</label>
            <input onChange={handleChipsFormInput} type="number" name="value" className="chipElement__field__inputNumber"
        ...
        <div className="chipElement__field">
            ...
            <input onChange={handleChipsFormInput} type="number" name="quantity" className="chipElement__field__inputNumber"
            min={0} onWheel={(e) => e.target.blur()}
...
ChipsChipElement.propTypes = {
    color: PropTypes.string.isRequired,
    value: PropTypes.number.isRequired,
    quantity: PropTypes.number.isRequired,
    handleChipsFormInput: PropTypes.func.isRequired,
...
const mapStateToProps = (state, ownprops) => {
    return {
        ...
        value: parseInt(state.chip.chips[ownprops.index].value, 10),
        quantity: parseInt(state.chip.chips[ownprops.index].quantity, 10),
        chipColor: state.chip.chips[ownprops.index].color,
    }
}
const mapDispatchToProps = (dispatch, ownprops) => {
    return {
    handleChipsFormInput: (event) => {
        dispatch({type: "CHANGE_CHIP_INPUTS", newValue: event.target.value, inputName: event.target.name, index: ownprops.index})
    },
    handleRemoveChip: (event) => {
        event.preventDefault()
        dispatch({type: "REMOVE_CHIP_CHIP", index: ownprops.index })
    },
}}

export default connect(mapStateToProps, mapDispatchToProps)(ChipsChipElement);
